...
def room_list(request):
    rooms = Room.objects.all().prefetch_related('booking_set')  # Optimize query
    ...
    for room in rooms:
        ...
        booked_dates = [(b.check_in, b.check_out) for b in bookings]

        # Determine room status based on bookings
        room.status = "Occupied" if bookings.exists() else "Available"

        # Find next available date
        today = date.today()
        next_available = today  # Default to today

        for check_in, check_out in sorted(booked_dates):
            if today < check_in:
                next_available = today  # Available today
                break
            elif today < check_out:
                next_available = check_out  # Available after last booking
            else:
                next_available = today  # No future bookings

        room.booked_dates = booked_dates  # Attach booked dates
        room.next_available = next_available  # Attach next available date
